...
class ChatbotController extends Controller
{
    ...
    public function chat(Request $request)
    {
        try {
            ...
            // If no API key in env, try to get from config
            if (empty($apiKey)) {
                $apiKey = config('services.openrouter.api_key');
            }

            // If still no key, use fallback only
            if (empty($apiKey)) {
                Log::warning('OpenRouter API key not configured, using fallback only');
    ...
    private function getLocalFallback($message)
    {
        ...
        if ($isTagalog) {
            ...
            if (preg_match('/book|booking|paano mag book|paano magpabook|paano mag reserve|reservation|reserve|unsaon pag book/', $msg)) {
                return "<strong>Paano mag-book sa SureCargo:</strong><br><br>1. Pumunta sa Dashboard, pumili ng AVAILABLE na truck<br>2. Ilagay ang quantity (egg trays), pickup address, receiver name/phone, drop-off location<br>3. I-submit, maghintay ng admin approval (within 24 oras)<br>4. Kapag 'confirmed' na, lalabas ang payment button para sa advance payment<br>5. Kapag 'in_transit' na ang status, lalabas ang Track button para sa live tracking<br><br>Araw ng booking: <strong>Sabado - Linggo, 9:00 AM onwards.</strong><br><br>Location: Mohon, Santa Fe, Cebu<br>Maintenance: Sungko, Bantayan, Cebu";
            }
            ...
            if (preg_match('/capacity|max trays|limit|truck|helper|driver|personel|personnel/', $msg)) {
                return "<strong>Impormasyon sa Truck:</strong><br><br>• Ang bawat truck ay kayang mag-hold ng <strong>12,000 egg trays</strong><br>• Personnel: <strong>1 driver</strong> at <strong>3 truck helpers</strong><br>• Ang mga helper ay masipag at maingat sa paghawak ng itlog<br><br>Driver: Nagse-send ng location through phone GPS";
            }
            ...
            if (preg_match('/edit|modify|update booking/', $msg)) {
                return "<strong>Edit Booking:</strong><br><br>Pwede mong i-edit ang booking habang ang status ay <strong>'pending'</strong>.<br><br>Kapag <strong>'confirmed'</strong> na, HINDI na pwede i-edit at lalabas na ang payment button.";
            }
        ...
        if (preg_match('/book|booking|how to book|reserve|schedule/', $msg)) {
            if (strpos($msg, 'step') !== false || strpos($msg, 'process') !== false || strpos($msg, 'how') !== false) {
                return "<strong>Booking Steps:</strong><br><br>1. Dashboard, select <strong>AVAILABLE</strong> truck<br>2. Enter quantity (egg trays), pickup address, receiver name/phone, drop-off location<br>3. Submit, wait admin approval (24h)<br>4. Once <strong>'confirmed'</strong>, Payment button appears for advance payment<br>5. When status is <strong>'in_transit'</strong>, Track button shows for live tracking<br><br>Booking days: <strong>Saturday - Sunday, 9:00 AM onwards.</strong><br><br>Location: Mohon, Santa Fe, Cebu<br>Maintenance: Sungko, Bantayan, Cebu";
            }
        ...
        // Truck & Personnel
        if (preg_match('/capacity|max trays|limit|truck|helper|driver|personel|personnel/', $msg)) {
            return "<strong>Truck Information:</strong><br><br>• Each truck holds <strong>12,000 egg trays</strong><br>• Personnel: <strong>1 driver</strong> and <strong>3 truck helpers</strong><br>• Helpers handle eggs with care and hard work<br><br>Driver: Sends location through phone GPS";
        }

        // Routes
        if (preg_match('/route|routes|way|path|city|bantayan|bacolod|escalante|sagay|cadiz|manapla|victorias|silay|bata|libertad/', $msg)) {
            return "<strong>SureCargo Routes:</strong><br><br><strong>From Bantayan Island to Bacolod City</strong><br>• One-way: Bantayan, Escalante, Sagay, Cadiz, Manapla, Victorias, Silay, Bata, Bacolod<br>• Includes Libertad cities<br><br><strong>Return Route:</strong><br>• Bacolod, Bata, Silay, Victorias, Manapla, Cadiz, Sagay, Escalante, Bantayan<br><br>Base: <strong>Sungko, Bantayan, Cebu</strong><br>Location: <strong>Mohon, Santa Fe, Cebu</strong>";
        }
        ...
        // Edit booking
        if (preg_match('/edit|modify|update booking/', $msg)) {
            return "<strong>Edit Booking:</strong><br><br>You can edit a booking only while status is <strong>'pending'</strong>.<br><br>Once <strong>'confirmed'</strong>, you CANNOT edit anymore and the payment button appears.";
        }
